...
class GeneralPreferencesFragment : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        clearCachePreference?.setOnPreferenceClickListener {
            viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                try {
                    ...
                    com.neubofy.remotemanager.data.CacheManager.clearCache(requireContext())
                    com.neubofy.remotemanager.data.ThumbnailCacheManager.clear(requireContext())
        ...
        clearTelemetryCachePreference?.setOnPreferenceClickListener {
            viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                try {
                    com.neubofy.remotemanager.data.RemoteTelemetryCacheRepository.clear(requireContext())
                    com.neubofy.remotemanager.data.DirectoryCacheRepository.clear(requireContext())
                    withContext(Dispatchers.Main) {
    ...
    private fun setupDefaultOpeners() {
        ...
        fun updateOpenerSummaries() {
            ...
            imagePref?.summary = com.neubofy.remotemanager.util.DefaultOpenerHelper.getOpenerSummary(ctx, imageKey)
            videoPref?.summary = com.neubofy.remotemanager.util.DefaultOpenerHelper.getOpenerSummary(ctx, videoKey)
            audioPref?.summary = com.neubofy.remotemanager.util.DefaultOpenerHelper.getOpenerSummary(ctx, audioKey)
            docPref?.summary = com.neubofy.remotemanager.util.DefaultOpenerHelper.getOpenerSummary(ctx, docKey)
        }

        updateOpenerSummaries()

        imagePref?.setOnPreferenceClickListener {
            com.neubofy.remotemanager.util.DefaultOpenerHelper.showAppPickerDialog(
        ...
        resetPref?.setOnPreferenceClickListener {
            com.neubofy.remotemanager.util.DefaultOpenerHelper.resetAllOpeners(requireContext())
            updateOpenerSummaries()
